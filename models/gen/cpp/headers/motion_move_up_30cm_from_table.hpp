...
struct motion_move_up_30cm_from_table_state {
    arm_solver_solver_state arm_solver;
    bool snapshot_taken = false;
    motion_spec::runtime::PIDControl ctrl_move_up_z{1.0, 0.0, 0.1};
    ...
    bool monitor_table_clearance_previous = false;
};

inline void reset_motion_move_up_30cm_from_table(motion_move_up_30cm_from_table_state &state) {
    state = motion_move_up_30cm_from_table_state{};
}

inline void init_motion_move_up_30cm_from_table(motion_move_up_30cm_from_table_state &state, const robot_io &robot) {
...
inline void update_motion_move_up_30cm_from_table(
    motion_move_up_30cm_from_table_state &state,
    shared_data &shared,
    const robot_io &robot) {
    init_motion_move_up_30cm_from_table(state, robot);
...
inline bool can_start_motion_move_up_30cm_from_table(
    motion_move_up_30cm_from_table_state &state,
    shared_data &shared) {
    return true;
}

inline void monitor_motion_move_up_30cm_from_table(
    motion_move_up_30cm_from_table_state &state,
    shared_data &shared) {
    // compute_inverse_pose_world_table
    shared.inverse_pose_world_table = shared.pose_world_table.Inverse();
    // compute_pose_table_ee
    shared.pose_table_ee = shared.inverse_pose_world_table * shared.pose_world_ee;
    // compute_pose_table_ee_distance
    shared.pose_table_ee_distance = shared.pose_table_ee.p.Norm();
    // eval_move_up_30cm_from_table_until_reached_table_clearance
    shared.pose_table_ee_distance_err = motion_spec::runtime::evaluate_greater_than_constraint(shared.pose_table_ee_distance, shared.table_clearance);

    {
        const bool active = motion_spec::runtime::constraint_satisfied(shared.pose_table_ee_distance_err);
        if (motion_spec::runtime::rising_edge(state.monitor_table_clearance_previous, active)) {
            motion_spec::runtime::warn_produce_event_not_implemented("table_clearance_reached");
        }
    }

}

inline void control_motion_move_up_30cm_from_table(
    motion_move_up_30cm_from_table_state &state,
    shared_data &shared,
    const robot_io &robot) {
    // eval_move_up_30cm_from_table_while_move_up_z
    shared.twist_ee_base_linear_z_err_move_up_30cm_from_table = motion_spec::runtime::evaluate_equality_constraint(shared.twist_ee_base.vel[2], shared.up_vel);
    ...
    // ctrl_move_up_z
    shared.eacc_twist_ee_base_linear_z_move_up_30cm_from_table = state.ctrl_move_up_z.control(shared.twist_ee_base_linear_z_err_move_up_30cm_from_table);
    ...
    state.arm_solver.e_acc(0) = shared.eacc_twist_ee_base_linear_z_move_up_30cm_from_table;
...
inline void apply_motion_move_up_30cm_from_table(
    motion_move_up_30cm_from_table_state &state,
    shared_data &shared,
    const robot_io &robot) {
    for (int i = 0; i < state.arm_solver.num_joints; ++i) {
