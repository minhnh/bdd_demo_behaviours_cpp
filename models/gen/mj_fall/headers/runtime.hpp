...
class PIDControl {
  public:
    ...
    double control_with_vel(double pos_error, double velocity) {
        err_integ = decay_rate * err_integ + (1.0 - decay_rate) * pos_error;
        err_last = pos_error;
        return kp * pos_error + ki * err_integ - kd * velocity;
    }
