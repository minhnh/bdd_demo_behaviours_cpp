...
struct motion_grasp_hold__open_grip_state {
    bool active = false;
    arm_solver_solver_state arm_solver;
    bool snapshot_taken = false;
    motion_spec::runtime::PIDControl ctrl_og_hold_x{100.0, 10.0, 20.0, 0.9};
    motion_spec::runtime::PIDControl ctrl_og_hold_y{100.0, 10.0, 20.0, 0.9};
    motion_spec::runtime::PIDControl ctrl_og_hold_z{100.0, 10.0, 20.0, 0.9};
    motion_spec::runtime::PIDControl ctrl_og_align_roll{80.0, 5.0, 40.0, 0.9};
    motion_spec::runtime::PIDControl ctrl_og_align_pitch{80.0, 5.0, 40.0, 0.9};
    motion_spec::runtime::PIDControl ctrl_og_align_yaw{80.0, 5.0, 40.0, 0.9};
    motion_spec::runtime::ImpedanceControl ctrl_og_support_z{800.0, 80.0};
    bool mon_at_place_previous = false;
};

inline void reset_motion_grasp_hold__open_grip(motion_grasp_hold__open_grip_state &state) {
    state = motion_grasp_hold__open_grip_state{};
}

inline void init_motion_grasp_hold__open_grip(motion_grasp_hold__open_grip_state &state, const robot_io &robot) {
...
inline void update_motion_grasp_hold__open_grip(
    motion_grasp_hold__open_grip_state &state,
    shared_data &shared,
    const robot_io &robot) {
    init_motion_grasp_hold__open_grip(state, robot);
...
inline bool can_start_motion_grasp_hold__open_grip(
    motion_grasp_hold__open_grip_state &state,
    shared_data &shared) {
    ...
    return true&& motion_spec::runtime::constraint_satisfied(shared.pose_cube_g_pinch_distance_err);
}

inline void monitor_motion_grasp_hold__open_grip(
    motion_grasp_hold__open_grip_state &state,
    shared_data &shared) {
    // compute_inverse_pose_ee_base
    shared.inverse_pose_ee_base = shared.pose_ee_base.Inverse();
...

    {
        ...
        if (motion_spec::runtime::rising_edge(state.mon_at_place_previous, active)) {
            motion_spec::runtime::warn_produce_event_not_implemented("at_place");
        }
    }

}

inline void control_motion_grasp_hold__open_grip(
    motion_grasp_hold__open_grip_state &state,
    shared_data &shared,
    const robot_io &robot) {
    {
        ...
        shared.pose_ee_wrt_cube_distance_x_err_grasp_hold = _pose_axis_error_linear_X;
        shared.pose_ee_wrt_cube_distance_y_err_grasp_hold = _pose_axis_error_linear_Y;
        shared.pose_ee_wrt_cube_distance_z_err_grasp_hold = _pose_axis_error_linear_Z;
        shared.pose_ee_wrt_cube_rotation_x_err_grasp_hold = _pose_axis_error_angular_X;
        shared.pose_ee_wrt_cube_rotation_y_err_grasp_hold = _pose_axis_error_angular_Y;
        shared.pose_ee_wrt_cube_rotation_z_err_grasp_hold = _pose_axis_error_angular_Z;
    }
    // eval_grasp_hold_while_support_elbow_z
    shared.pose_elbow_base_distance_z_err_grasp_hold = motion_spec::runtime::evaluate_equality_constraint(shared.support_z, shared.pose_elbow_base.p[2]);
    // ctrl_og_support_z
    shared.force_ctrl_og_support_z = state.ctrl_og_support_z.control(shared.pose_elbow_base_distance_z_err_grasp_hold);
    // ctrl_og_align_yaw
    shared.eacc_pose_ee_wrt_cube_rotation_z_grasp_hold = state.ctrl_og_align_yaw.control(shared.pose_ee_wrt_cube_rotation_z_err_grasp_hold);
    // ctrl_og_align_pitch
    shared.eacc_pose_ee_wrt_cube_rotation_y_grasp_hold = state.ctrl_og_align_pitch.control(shared.pose_ee_wrt_cube_rotation_y_err_grasp_hold);
    // ctrl_og_align_roll
    shared.eacc_pose_ee_wrt_cube_rotation_x_grasp_hold = state.ctrl_og_align_roll.control(shared.pose_ee_wrt_cube_rotation_x_err_grasp_hold);
    // ctrl_og_hold_z
    shared.eacc_pose_ee_wrt_cube_distance_z_grasp_hold = state.ctrl_og_hold_z.control(shared.pose_ee_wrt_cube_distance_z_err_grasp_hold);
    // ctrl_og_hold_y
    shared.eacc_pose_ee_wrt_cube_distance_y_grasp_hold = state.ctrl_og_hold_y.control(shared.pose_ee_wrt_cube_distance_y_err_grasp_hold);
    // ctrl_og_hold_x
    shared.eacc_pose_ee_wrt_cube_distance_x_grasp_hold = state.ctrl_og_hold_x.control(shared.pose_ee_wrt_cube_distance_x_err_grasp_hold);
    // compute_direction_ctrl_og_support_z
    shared.direction_ctrl_og_support_z = shared.pose_elbow_base.p;
    shared.direction_ctrl_og_support_z.Normalize();
    ...
    state.arm_solver.e_acc(0) = shared.eacc_pose_ee_wrt_cube_distance_x_grasp_hold;
    ...
    state.arm_solver.e_acc(1) = shared.eacc_pose_ee_wrt_cube_distance_y_grasp_hold;
    ...
    state.arm_solver.e_acc(2) = shared.eacc_pose_ee_wrt_cube_distance_z_grasp_hold;
    ...
    state.arm_solver.e_acc(3) = shared.eacc_pose_ee_wrt_cube_rotation_x_grasp_hold;
    ...
    state.arm_solver.e_acc(4) = shared.eacc_pose_ee_wrt_cube_rotation_y_grasp_hold;
    ...
    state.arm_solver.e_acc(5) = shared.eacc_pose_ee_wrt_cube_rotation_z_grasp_hold;
...
inline void apply_motion_grasp_hold__open_grip(
    motion_grasp_hold__open_grip_state &state,
    shared_data &shared,
    const robot_io &robot) {
    for (int i = 0; i < state.arm_solver.num_joints; ++i) {
    ...
    }
    {
        ...
        if (actuator_id >= 0) {
            robot.arm_solver.robot->data->ctrl[actuator_id] = 0.0;
        }
    }
